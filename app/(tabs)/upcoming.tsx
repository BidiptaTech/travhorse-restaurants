...
export default function UpcomingScreen() {
  ...
  const { upcoming: orders, loading, error } = useAppSelector((s) => s.orders);
  ...
  const rowBg = isDark ? "#1D1F24" : "#f0f9ff";
  ...
  // Fetch upcoming orders every time the screen is focused (e.g. when user clicks Upcoming button)
  useFocusEffect(
    useCallback(() => {
  ...
  const onRefresh = async () => {
    setRefreshing(true);
    await dispatch(fetchRestaurantOrders("upcoming"));
    setRefreshing(false);
  };
  ...
  return (
    <SafeAreaView className="flex-1" style={{ backgroundColor: bg }} edges={["left", "right", "bottom"]}>
      <WaveScreenHeader title="Upcoming events" showBackButton />
            ...
            <View ref={dmcTriggerRef} collapsable={false}>
              <TouchableOpacity
                ...
                style={{
                  backgroundColor: isDark ? "#1D1F24" : "#e0f2fe",
                  borderWidth: 1,
                  borderColor: isDark ? "#2A2B30" : "#bae6fd",
                }}
                activeOpacity={0.7}
              >
                <Text
                ...
                >
                  ...
                  >
                    <Text
                      ...
                      style={{
                        color: selectedDmc === null ? "#2563eb" : textPrimary,
                      }}
                    >
                      All
                    </Text>
                  ...
                  {filteredDmcNames.map((dmcName) => (
                    ...
                    >
                      <Text
                        ...
                        style={{
                          color:
                            selectedDmc === dmcName ? "#2563eb" : textPrimary,
                        }}
                        numberOfLines={1}
                      >
                        {dmcName}
          ...
          <>
            ...
            {filteredOrders.length === 0 && !error && (
              <View className="items-center justify-center mt-16">
                <Ionicons
                  name={
                    searchQuery.trim() ? "search-outline" : "calendar-outline"
                  }
                  size={32}
                  color={textSecondary}
                />
